...
export function view(date, state) {
  ...
  const rows = items.map(item => {
    ...
    if (item.method === 'select') return `<div class="row"><div><div class="item-name">${esc(item.name)}</div><div class="muted">${esc(item.unit || 'Value')}</div></div><select data-feature="daily-habits" data-field="value" data-id="${item.id}"><option value="">—</option>${options(item.options, r?.value)}</select></div>`;
    return `<div class="row"><div class="item-name">${esc(item.name)}</div><input class="check" type="checkbox" data-feature="daily-habits" data-field="done" data-id="${item.id}" ${r?.done ? 'checked' : ''}></div>`;
  }).join('');
