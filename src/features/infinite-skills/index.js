...
export function view(date, state) {
  ...
  const rows = items.map(item => {
    ...
    return `<div class="skill-block"><div class="row"><div class="item-name">${esc(item.name)}</div><div class="control">${t.completed ? `<label class="muted">Done <input class="check" type="checkbox" data-feature="infinite-skills" data-field="done" data-id="${esc(item.id)}" ${r.done ? 'checked' : ''}></label>` : ''}${t.time ? `<input class="number" type="number" min="0" step="1" placeholder="min" value="${r.minutes ?? ''}" data-feature="infinite-skills" data-field="minutes" data-id="${esc(item.id)}">` : ''}${t.repetitions ? `<input class="number" type="number" min="0" step="1" placeholder="reps" value="${r.repetitions ?? ''}" data-feature="infinite-skills" data-field="repetitions" data-id="${esc(item.id)}">` : ''}${t.rounds ? `<input class="number" type="number" min="0" step="1" placeholder="rounds" value="${r.rounds ?? ''}" data-feature="infinite-skills" data-field="rounds" data-id="${esc(item.id)}">` : ''}</div></div>${t.note ? `<input class="inline-note" data-feature="infinite-skills" data-field="note" data-id="${esc(item.id)}" placeholder="Optional note" value="${esc(r.note || '')}">` : ''}</div>`;
  }).join('');
